...
class Nav {
  ...
  _init() {
    ...
    this._onHorizontalMenuDropdownHidden = this._onHorizontalMenuDropdownHidden.bind(
      this,
    );
    this._onHorizontalMenuDropdownClick = this._onHorizontalMenuDropdownClick.bind(
      this,
    );
  ...
  _addListeners() {
    ...
    this.html.addEventListener(Globals.menuPlacementChange, (event) =>
      this._onMenuPlacementChange(event),
    );
    this.html.addEventListener(Globals.menuBehaviourChange, (event) =>
      this._onMenuBehaviourChange(event),
    );
      ...
      if (
        ...
        if (element.getAttribute('data-bs-target')) {
          element.nextElementSibling.setAttribute(
            'id',
            element.getAttribute('data-bs-target').substring(1),
          );
        }
      }
    });
  ...
  _onHorizontalMenuDropdownClick(event) {
    ...
    if (openSubmenu && !element.nextElementSibling.classList.contains('show')) {
      ...
      openSubmenu
        ...
        .forEach((openSubMenuDropdown) => {
          openSubMenuDropdown.classList.remove('show');
        });
      openSubmenu
        .querySelectorAll('.dropdown-menu')
        .forEach((openSubMenuDropdownMenu) => {
    ...
    document.querySelectorAll(selector + ' a').forEach((el) => {
      ...
      const hrefData =
        ...
        el
          ...
          .replace(/\.[^/.]+$/, '');
      if (
        urlWithoutBackslash.includes(href) ||
        urlWithoutBackslash.includes(hrefData)
      ) {
  ...
  _initMenuBehaviour() {
    ...
    if (
      ...
      this.selectedMenuBehaviour === 'unpinned'
    ) {
      if (
        this.verticalMobile > windowWidth ||
        this.verticalUnpinned <= windowWidth
      ) {
        // Mobile and Desktop
    ...
    if (
      menuPlacement === 'vertical' &&
      this.selectedMenuBehaviour === 'pinned'
    ) {
      if (
        this.verticalMobile > windowWidth ||
        this.verticalUnpinned <= windowWidth
    ...
    if (
      ...
      document.querySelectorAll('#menu>li>a').forEach((element) => {
        if (
          element.nextElementSibling &&
          element.nextElementSibling.tagName === 'UL' &&
          element.nextElementSibling.classList.contains('collapsing')
        ) {
          collapsing = true;
        }
      });
  ...
  _onWindowScrollInterval() {
    if (this.windowScrolled) {
      ...
      this.windowScrolled = false;
      if (
        Math.abs(this.prevScrollpos - currentScrollPos) <= navSize &&
        currentScrollPos > navSize
      ) {
