...
class Mailbox {
  ...
  constructor(options = {}) {
    ...
    this.emailCorrespondenceContainer = document.getElementById(
      'emailCorrespondenceContainer',
    );
    this.emailListTemplate = document.getElementById('emailListTemplate');
    this.emailListBadgeTemplate = document.getElementById(
      'emailListBadgeTemplate',
    );
    ...
    this.emailMenuModal = new bootstrap.Modal(
      document.getElementById('emailMenuModal'),
    );
    this.newEmailButton = document.getElementById('newEmailButton');
    this.newEmailModal = new bootstrap.Modal(
      document.getElementById('newEmailModal'),
    );

    this.currentShowSettings = {
      folder: 'inbox',
      starred: 'All',
      important: 'All',
      tags: 'All',
    }; // Items are rendered based on this variable which is retrieved from the menu items upon click
    this.activeMenuId = 1;
  ...
  _renderItems(data) {
    ...
    renderData[showSettings.folder].map((email) => {
      ...
      if (
        (showSettings.starred === email.starred ||
          showSettings.starred.toString().includes('All')) &&
        (showSettings.important === email.important ||
          showSettings.important.toString().includes('All')) &&
        (tagString.includes(showSettings.tags) ||
          showSettings.tags.includes('All'))
      ) {
  ...
  _renderListItem(email) {
    var itemClone = this.emailListTemplate.content
      .cloneNode(true)
      .querySelector('div');
    itemClone.setAttribute('data-id', email.id);
    if (email.emails.length > 1) {
      itemClone.querySelector('.from').innerHTML =
        email.from + ' (' + email.emails.length + ')';
    } else {
    ...
    email.tags.map((tag) => {
      var badgeClone = this.emailListBadgeTemplate.content
        .cloneNode(true)
        .querySelector('span');
  ...
  _addListeners() {
    this.emailListContainer &&
      this.emailListContainer.addEventListener(
        'click',
        this._onEmailClick.bind(this),
      );
    this.emailMenuButton &&
      this.emailMenuButton.addEventListener(
        'click',
        this._showEmailMenuModal.bind(this),
      );
    this.newEmailButton &&
      this.newEmailButton.addEventListener(
        'click',
        this._showNewEmailModal.bind(this),
      );
    this.backButton &&
      this.backButton.addEventListener(
        'click',
        this._backButtonClick.bind(this),
      );
    this.replyButton &&
      this.replyButton.addEventListener(
        'click',
        this._replyButtonClick.bind(this),
      );
    this.forwardButton &&
      this.forwardButton.addEventListener(
        'click',
        this._forwardButtonClick.bind(this),
      );
    this.replyDeleteButton &&
      this.replyDeleteButton.addEventListener(
        'click',
        this._replyDeleteButtonClick.bind(this),
      );
    this.emailSearch &&
      this.emailSearch.addEventListener(
        'keydown',
  ...
  _initMoveContent() {
    if (
      document.querySelector('#emailMoveContent') &&
      typeof MoveContent !== 'undefined'
    ) {
      const mailboxMenuMove = document.querySelector('#emailMoveContent');
      const targetSelectorMenu = mailboxMenuMove.getAttribute(
        'data-move-target',
      );
      const moveBreakpointMenu = mailboxMenuMove.getAttribute(
        'data-move-breakpoint',
      );
  ...
  _onSearchKeyDown(event) {
    ...
    this.fuse = new Fuse(
      this.emailData[this.currentShowSettings.folder],
      this.fuseOptions,
    );
