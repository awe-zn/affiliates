...
class Tasks {
  ...
  _renderItems() {
    ...
    this.taskData.map((task) => {
      ...
      if (
        ...
        (showSettings.status.includes(task.status) ||
          showSettings.status.includes('All')) &&
  ...
  _renderNoResult(data) {
    if (data.length === 0) {
      var itemClone = this.noTasksFoundTemplate.content
        .cloneNode(true)
        .querySelector('div');
  ...
  _renderItem(task) {
    var itemClone = this.taskItemTemplate.content
      .cloneNode(true)
      .querySelector('div');
    ...
    task.tags.map((tag) => {
      var tagClone = this.taskTagTemplate.content
        .cloneNode(true)
        .querySelector('span');
  ...
  _initMoveContent() {
    if (document.querySelector('#tasksMenuMoveContent')) {
      ...
      const moveBreakpointMenu = tasksMenuMove.getAttribute(
      ...
      if (jQuery(item.element).val()) {
        return jQuery(
          '<span><span class="align-middle d-inline-block ms-1 ' +
            jQuery(item.element).data('selectionColor') +
            '">' +
            item.text +
            '</span></span>',
        );
  ...
  _editTaskById(id) {
    ...
    jQuery('#newTaskStatus').trigger('change');
    jQuery('#newTaskTags')
      .val(this._getTagsWithoutClasses(task.tags))
      .trigger('change');
    this.newTaskModal.show();
  ...
  _showRegularDropdownItems() {
    ...
    document
      .querySelectorAll('.dropdown-item.delete-task')
      .forEach((element) => {
        element.classList.remove('d-none');
      });
  }
