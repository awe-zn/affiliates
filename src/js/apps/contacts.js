...
class Contacts {
  ...
  _addListeners() {
    this.contactContainer.addEventListener(
      'click',
      this._onContainerClick.bind(this),
    );
    document.getElementById('newContactButton') &&
      document
        .getElementById('newContactButton')
        .addEventListener('click', this._showAddModal.bind(this));
    document.getElementById('deleteContact') &&
      document
        .getElementById('deleteContact')
        .addEventListener('click', this._deleteContact.bind(this));
    document.getElementById('addContact') &&
      document
        .getElementById('addContact')
        .addEventListener('click', this._addContact.bind(this));
    document.getElementById('saveContact') &&
      document
        .getElementById('saveContact')
        .addEventListener('click', this._saveContact.bind(this));
    document.getElementById('deleteConfirmButton') &&
      document
        .getElementById('deleteConfirmButton')
        .addEventListener('click', this._deleteContactConfirm.bind(this));
    document.getElementById('deleteChecked') &&
      document
        .getElementById('deleteChecked')
        .addEventListener('click', this._deleteChecked.bind(this));
  }
  ...
  _showDetailModal() {
    document.getElementById(
      'contactNameModal',
    ).value = this.currentItem.values().name;
    document.getElementById(
      'contactPositionModal',
    ).value = this.currentItem.values().position;
    document.getElementById(
      'contactEmailModal',
    ).value = this.currentItem.values().email;
    document.getElementById(
      'contactPhoneModal',
    ).value = this.currentItem.values().phone;
    document.getElementById(
      'contactGroupModal',
    ).value = this.currentItem.values().group;
    document
      .getElementById('contactThumbModal')
      .setAttribute('src', this.currentItem.values().thumb);
  ...
  _deleteContact(event) {
    ...
    document.getElementById(
      'deleteConfirmDetail',
    ).innerHTML = this.currentItem.values().name;
  ...
  _deleteContactConfirm(event) {
    ...
    const checkAllInput = document.querySelector(
      '.check-all-container-checkbox-click .btn-custom-control input',
    );
  ...
  _clearAddEditModal() {
    ...
    document
      .getElementById('contactThumbModal')
      .setAttribute('src', this.settings.emptyThumb);
    jQuery('#contactGroupModal').trigger('change');
  }
  ...
  _hideElement(selector) {
    document.getElementById(selector) &&
      document.getElementById(selector).classList.remove('d-inline-block');
    document.getElementById(selector) &&
