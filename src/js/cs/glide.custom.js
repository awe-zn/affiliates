  ...
  _init() {
    ...
    if (this.addDots) {
      this._glide.on(['mount.before'], () => {
        const bulletCount = this._glide.selector.querySelectorAll(
          '.glide__slide',
        ).length;
        const bulletWrapper = this._glide.selector.querySelectorAll(
          '.glide__bullets',
        )[0];
    ...
    this._glide.on(['resize', 'build.after'], () => {
      ...
      const total = this._glide.selector.querySelectorAll('.glide__slide')
        .length;
