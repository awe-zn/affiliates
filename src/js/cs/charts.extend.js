...
class ChartsExtend {
    ...
    const tooltips = {
      ...
      custom: function (tooltip) {
        ...
        if (tooltip.body) {
          ...
          icon.setAttribute(
            'data-cs-icon',
            chart._data.datasets[datasetIndex].icons[index],
          );
          csicons.replace();
          container.querySelector('.title').innerHTML =
            chart._data.datasets[datasetIndex].label;
          container.querySelector('.text').innerHTML = chart._data.labels[
            index
          ].toLocaleUpperCase();
          container.querySelector('.value').innerHTML = Helpers.AddCommas(
    ...
    if (!Chart.defaults.doughnutSmall) {
      ...
      Chart.controllers.doughnutSmall = Chart.controllers.doughnut.extend({
        draw: function (ease) {
          ...
          var gap =
            ((firstSegment.outerRadius - firstSegment.innerRadius) *
              (1 - 0.3)) /
            2;
          ctx.save();
        ...
        options: {
          ...
          legendCallback: function (chart) {
            ...
            itemClone.querySelector(
              '.text',
            ).innerHTML = chart.data.labels[0].toLocaleUpperCase();
            itemClone.querySelector('.value').innerHTML =
              chart.data.datasets[0].data[0] +
              ' / ' +
              (chart.data.datasets[0].data[1] + chart.data.datasets[0].data[0]);
  ...
  static SmallLineChart(chartId, chartData) {
    const tooltips = {
      enabled: false,
      custom: function (tooltip) {
        var tooltipEl = this._chart.canvas.parentElement.parentElement.querySelector(
          '.custom-tooltip',
        );
        if (tooltip.body) {
          var chart = this;
          ...
          csicons.replace();
          tooltipEl.querySelector('.title').innerHTML =
            chart._data.datasets[datasetIndex].label;
          tooltipEl.querySelector('.text').innerHTML =
            chart._data.labels[index];
          tooltipEl.querySelector('.value').innerHTML = Helpers.AddCommas(
            chart._data.datasets[datasetIndex].data[index],
          );
        }
