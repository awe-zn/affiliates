...
class AutocompleteCustom {
  ...
  _extendProps() {
    ...
    this.settings.onSelection =
      this.settings.onSelection ||
      ((feedback) => {
        document.getElementById(this._searchInputId).value =
          feedback.selection.value || feedback.selection.value['label'];
  ...
  _addListeners() {
    document
      .getElementById(this._searchInputId)
  ...
  _onInputFocusOut() {
    ...
    document
      .getElementById(this._searchInputId)
      .removeEventListener('keyup', this._onKeyUp.bind(this));
    this._hideResults();
  }
